...
Let us write \(i = g\circ f\). This function is injective on \(A\) and it's range is \(C\). We can similarly define \(j = f \circ g\) which will also be injective on \(B\) and whose range is \(D\).
...
We say sets \(A\) and \(B\) have the same cardinality if tthere exists a bijection \(f: A \rightarrow B\). We write this \(|A|=|B|\).
...
The continuum hypothesis is the conjecture that \(|\mathbb{R}| = |\mathbb{P}(\mathbb{N})|\). This is famous because Godel showed that it cannot be falsified in our standard model of set theory and Cohen showed that it cannot be proved either.
\end{remark}
